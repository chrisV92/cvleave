    ...
    <x-filament::section id="notifications" heading="{{ __('Ειδοποιήσεις') }}" icon="heroicon-o-bell">
        <div class="kb-section">
            ...
            <ul>
                ...
                <li>{{ __('σου ανατίθεται εργασία') }}</li>
                <li>{{ __('κάποιος σχολιάζει σε εργασία που σε αφορά') }}</li>
                <li>{{ __('μια εργασία σου λήγει αύριο ή έχει ξεπεράσει την προθεσμία') }}</li>
            </ul>
            <p><strong>{{ __('Ποτέ για δικές σου ενέργειες.') }}</strong> {{ __('Αν αναθέσεις εργασία στον εαυτό σου ή σχολιάσεις εσύ, δεν παίρνεις ειδοποίηση — θα ήταν θόρυβος για κάτι που μόλις έκανες.') }}</p>
            <p><strong>{{ __('Εβδομαδιαία σύνοψη:') }}</strong> {{ __('κάθε Δευτέρα πρωί λαμβάνεις email με τις ανοιχτές σου εργασίες, χωρισμένες σε εκπρόθεσμες, αυτής της εβδομάδας και αργότερα. Αν δεν έχεις τίποτα ανοιχτό, δεν στέλνεται.') }}</p>
            <p>{{ __('Από το προφίλ σου μπορείς να κλείσεις τα email και την εβδομαδιαία σύνοψη ξεχωριστά. Το κουδουνάκι μένει πάντα — εκεί κρατιέται το ιστορικό.') }}</p>
        </div>
    </x-filament::section>
